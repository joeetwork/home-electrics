...
export async function getDashboardData() {
  try {
    ...
    // Parse system data
    // API returns power in Watts (W) - keep as W for display
    const d = systemData?.data || {};

    const solar = {
      power: d.solar?.power || 0, // Watts
      arrays: d.solar?.arrays || [],
    };

    const battery = {
      percent: d.battery?.percent || 0,
      power: d.battery?.power || 0, // Watts
      temperature: d.battery?.temperature,
    };

    const grid = {
      power: d.grid?.power || 0, // Watts
    ...
    const consumption = {
      power: typeof d.consumption === 'number' ? d.consumption : d.consumption?.power || 0, // Watts
    };
    ...
    const inverter = {
      ...
      power: d.inverter?.power || 0, // Watts
    };
    ...
    const dataPoints = (dataPointsResponse?.data || []).map(point => ({
      time: point.time,
      solar: { power: point.power?.solar?.power || 0 }, // Watts
      consumption: { power: point.power?.consumption?.power || 0 }, // Watts
      battery: { power: point.power?.battery?.power || 0 }, // Watts
      grid: { power: point.power?.grid?.power || 0 }, // Watts
    }));
