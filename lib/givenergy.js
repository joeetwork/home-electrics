...
export async function getDashboardData() {
  try {
    ...
    // Parse system data
    // API returns power in Watts (W) and energy in Watt-hours (Wh)
    // Convert to kW and kWh for display
    ...
    const consumption = {
      power: (d.consumption?.power || 0) / 1000, // W -> kW
    };
    ...
    const todayStats = {
      solar: (d.today?.solar || 0) / 1000, // Wh -> kWh
      consumption: (d.today?.consumption || 0) / 1000, // Wh -> kWh
      import: (d.today?.grid?.import || 0) / 1000, // Wh -> kWh
      export: (d.today?.grid?.export || 0) / 1000, // Wh -> kWh
      batteryCharge: (d.today?.battery?.charge || 0) / 1000, // Wh -> kWh
      batteryDischarge: (d.today?.battery?.discharge || 0) / 1000, // Wh -> kWh
    };

    // Parse data points for chart
    const dataPoints = (dataPointsResponse?.data || []).map(point => ({
      time: point.time,
      solar: { power: (point.solar?.power || 0) / 1000 }, // W -> kW
      consumption: { power: (point.consumption?.power || 0) / 1000 }, // W -> kW
      battery: { power: (point.battery?.power || 0) / 1000 }, // W -> kW
      grid: { power: (point.grid?.power || 0) / 1000 }, // W -> kW
    }));
